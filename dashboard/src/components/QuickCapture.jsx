...
import { Send, CheckCircle2, XCircle, PencilLine } from 'lucide-react'
...
export default function QuickCapture() {
  ...
  const hint = useMemo(() => {
    return 'Hizli not...  (#gorev <baslik> = gorev)'
  }, [])
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setResult({ ok: false, message: data.error || 'Kaydedilemedi' })
        return
      }
      setResult({ ok: true, message: data.taskId ? `${data.taskId} olusturuldu` : 'Kaydedildi' })
      setText('')
    } catch (e) {
  ...
  return (
    <div className="rounded-xl ax-glass p-4">
      <div className="flex items-center gap-2 mb-3">
        <div className="p-1.5 rounded-md bg-ax-cyan/10"><PencilLine size={14} className="text-ax-cyan" /></div>
        <h2 className="text-xs font-bold uppercase text-ax-heading">Quick Capture</h2>
        {result && (
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="w-10 h-10 rounded-xl bg-ax-accent text-black flex items-center justify-center hover:brightness-110 disabled:opacity-50 disabled:hover:brightness-100 transition"
          title="Gonder"
        >
